!function() {
  ...
  var external_parser_namespaceObject = require("./parser"), external_ast_namespaceObject = require("./ast"), external_long_namespaceObject = require("./long"), external_long_default = __webpack_require__.n(external_long_namespaceObject);
  ...
  function shrinkPaddedLEB128(ast, uint8Buffer) {
    return (0, external_ast_namespaceObject.traverse)(ast, {
      SectionMetadata: function(_ref2) {
        ...
        if (newu32EncodedLen !== oldu32EncodedLen) {
          ...
          uint8Buffer = overrideBytesInBuffer(uint8Buffer, start, end, newu32Encoded), function(ast, _ref, deltaInSizeEncoding) {
            ...
            (0, external_ast_namespaceObject.traverse)(ast, {
              SectionMetadata: function(path) {
                path.node.section !== section ? !0 === encounteredSection && (0, external_ast_namespaceObject.shiftSection)(ast, path.node, deltaInSizeEncoding) : encounteredSection = !0;
              }
            });
  ...
  function createEmptySection(ast, uint8Buffer, section) {
    ...
    var sizeStartLoc = {
      ...
      line: -1,
      column: start + 1
    }, size = external_ast_namespaceObject.withLoc(external_ast_namespaceObject.numberLiteralFromRaw(1), sizeEndLoc, sizeStartLoc), vectorOfSizeStartLoc = {
      line: -1,
      column: sizeEndLoc.column
    }, vectorOfSizeEndLoc = {
      line: -1,
      column: sizeEndLoc.column + 1
    }, vectorOfSize = external_ast_namespaceObject.withLoc(external_ast_namespaceObject.numberLiteralFromRaw(0), vectorOfSizeEndLoc, vectorOfSizeStartLoc), sectionMetadata = external_ast_namespaceObject.sectionMetadata(section, start, size, vectorOfSize), sectionBytes = encodeNode(sectionMetadata);
    ...
    external_ast_namespaceObject.sortSectionMetadata(ast.body[0]));
    var deltaBytes = +sectionBytes.length, encounteredSection = !1;
    return external_ast_namespaceObject.traverse(ast, {
      SectionMetadata: function(path) {
        path.node.section !== section ? !0 === encounteredSection && external_ast_namespaceObject.shiftSection(ast, path.node, deltaBytes) : encounteredSection = !0;
      }
    }), {
  ...
  function shiftLocNodeByDelta(node, delta) {
    (0, external_ast_namespaceObject.assertHasLoc)(node), node.loc.start.column += delta, 
    node.loc.end.column += delta;
  }
  function applyDelete(ast, uint8Buffer, node) {
    if ((0, external_ast_namespaceObject.assertHasLoc)(node), "start" === getSectionForNode(node)) {
      var sectionMetadata = (0, external_ast_namespaceObject.getSectionMetadata)(ast, "start");
      return uint8Buffer = function(ast, uint8Buffer, section) {
        var sectionMetadatas = (0, external_ast_namespaceObject.getSectionMetadatas)(ast, section);
        ...
        return sectionMetadatas.reverse().reduce((function(uint8Buffer, sectionMetadata) {
          ...
          return (0, external_ast_namespaceObject.traverse)(ast, {
            SectionMetadata: function(path) {
              ...
              !0 === encounteredSection && (0, external_ast_namespaceObject.shiftSection)(ast, path.node, delta);
            }
    ...
    return ops.forEach((function(op) {
      ...
      switch (op.kind) {
       case "update":
        state = function(ast, uint8Buffer, _ref) {
          ...
          (0, external_ast_namespaceObject.assertHasLoc)(oldNode);
          ...
          "code" === sectionName && (0, external_ast_namespaceObject.traverse)(ast, {
            Func: function(_ref3) {
              var node = _ref3.node;
              if (!0 == (void 0 !== node.body.find((function(n) {
                return n === newNode;
              })))) {
                (0, external_ast_namespaceObject.assertHasLoc)(node);
       ...
       case "add":
        state = function(ast, uint8Buffer, node) {
          var body, sectionName = getSectionForNode(node), sectionMetadata = (0, external_ast_namespaceObject.getSectionMetadata)(ast, sectionName);
          ...
          if ((0, external_ast_namespaceObject.isFunc)(node) && (0 === (body = node.body).length || "end" !== body[body.length - 1].id)) throw new Error("expressions must be ended");
          if ((0, external_ast_namespaceObject.isGlobal)(node) && (0 === (body = node.init).length || "end" !== body[body.length - 1].id)) throw new Error("expressions must be ended");
          var newByteArray = encodeNode(node), start = (0, external_ast_namespaceObject.getEndOfSection)(sectionMetadata), end = start, deltaBytes = newByteArray.length;
          ...
          return "IndexInFuncSection" !== node.type && (0, external_ast_namespaceObject.orderedInsertNode)(ast.body[0], node), 
          {
            uint8Buffer: uint8Buffer,
            deltaBytes: deltaBytes,
            deltaElements: 1
      ...
      if (0 !== state.deltaElements && "start" !== sectionName) {
        ...
        state.uint8Buffer = function(ast, uint8Buffer, section, deltaElements) {
          var sectionMetadata = (0, external_ast_namespaceObject.getSectionMetadata)(ast, section);
      ...
      if (0 !== state.deltaBytes && "start" !== sectionName) {
        ...
        state.uint8Buffer = function(ast, uint8Buffer, section, deltaBytes) {
          ...
          return (0, external_ast_namespaceObject.traverse)(ast, {
            SectionMetadata: function(path) {
              path.node.section !== section ? !0 === encounteredSection && (0, external_ast_namespaceObject.shiftSection)(ast, path.node, deltaBytes) : encounteredSection = !0;
            }
          }), overrideBytesInBuffer(uint8Buffer, start, end, newBytes);
  ...
  function preprocess(ab) {
    return function(uint8Buffer) {
      try {
        return shrinkPaddedLEB128((0, external_parser_namespaceObject.decode)(uint8Buffer.buffer, decoderOpts), uint8Buffer);
      } catch (e) {
  ...
  function edit(ab, visitors) {
    return ab = preprocess(ab), editWithAST((0, external_parser_namespaceObject.decode)(ab), ab, visitors);
  }
  function editWithAST(ast, ab, visitors) {
    var nodeBefore, operations = [], uint8Buffer = new Uint8Array(ab);
    return (0, external_ast_namespaceObject.traverse)(ast, visitors, (function(type, path) {
      nodeBefore = (0, external_ast_namespaceObject.cloneNode)(path.node);
    }), (function(type, path) {
  ...
  function add(ab, newNodes) {
    return ab = preprocess(ab), addWithAST((0, external_parser_namespaceObject.decode)(ab), ab, newNodes);
  }
