              ...
              this.patterns.map(((item, index) => scheduledTasks.push((async () => {
                ...
                if (assets && assets.length > 0) {
                  if (item.transformAll) {
                    ...
                    const mergedEtag = 1 === assets.length ? cache.getLazyHashedEtag(assets[0].source.buffer()) : assets.reduce(((accumulator, asset, i) => accumulator = cache.mergeEtags(1 === i ? cache.getLazyHashedEtag(accumulator.source.buffer()) : accumulator, cache.getLazyHashedEtag(asset.source.buffer())))), cacheKeys = `transformAll|${(0, 
                    _serializeJavascript.default)({
                      version: _packageVersion,
                      from: item.from,
                      to: item.to,
                      transformAll: item.transformAll
                    })}`, eTag = cache.getLazyHashedEtag(mergedEtag), cacheItem = cache.getItemCache(cacheKeys, eTag);
