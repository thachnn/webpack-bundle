...
module.exports = [
      ...
      rules: [
        {
          ...
          loader: 'webpack/lib/replace-loader',
          options: { search: / require(\(\w+|\.resolve\b)/g, replace: ' __non_webpack_require__$1' },
        },
        {
  ...
  webpackConfig('@babel-preset', {
    entry: {
      'lib/core-js-compat': { import: './node_modules/core-js-compat/index', library: { type: 'commonjs2' } },
    ...
    externals: {
      ...
      'core-js-compat': 'commonjs2 ./core-js-compat',
    ...
    module: {
      rules: [
        ...
        {
          test: /node_modules.(@babel.preset-env.lib.normalize-options|babel-plugin-polyfill-corejs3.lib.[biu][\w-]*)\.js$/i,
          loader: 'webpack/lib/replace-loader',
          options: {
            search: /\brequire\("(?:\.+\/)?core-js-compat\/([\w-]+)[^"]*"\)/g,
            replace: (_m, p1) => `require("core-js-compat").${p1.camelize()}`,
          },
        },
        {
    ...
    plugins: [
      newCopyPlugin([
        ...
        {
          from: 'node_modules/{semver/semver.*,@babel/helper-plugin-utils/lib/index.js}',
          to: ({ absoluteFilename: f }) => `lib/${f.replace(/^.*\bnode_modules(.@[\w-]*)?[\\/]|[\\/].*$/g, '')}[ext]`,
        },
      ]),
  ...
  //
  webpackConfig('@babel-runtime', {
    entry: { 'lib/index': './node_modules/@babel/plugin-transform-runtime/lib/index' },
    output: { libraryTarget: 'commonjs' },
    target: 'node6.9',
    externals: {
      '@babel/core': 'commonjs @babel/core',
      '@babel/helper-plugin-utils': 'commonjs ./helper-plugin-utils',
      semver: 'commonjs2 ./semver',
      'core-js-compat': 'commonjs2 ./core-js-compat',
      browserslist: 'commonjs2 ./browserslist',
      '@babel/types': 'commonjs ./types',
    ...
    module: {
      rules: [
        {
          test: /node_modules.@babel.(plugin-transform-runtime.lib.get-runtime-path.index|helper-define-polyfill-provider.lib.node.dependencies)\.js$/i,
          loader: 'webpack/lib/replace-loader',
          options: { search: / require(\(\w+|\.resolve\b)/g, replace: ' __non_webpack_require__$1' },
        },
        {
    ...
    plugins: [
      newCopyPlugin([
        { from: 'node_modules/@babel/plugin-transform-runtime/{LICENSE*,README*}', to: '[name][ext]' },
        {
          from: 'node_modules/@babel/plugin-transform-runtime/package.json',
          transform: (content) => String(content).replace(/,\s*"d(evD)?ependencies": *\{[^{}]*\}/g, ''),
        },
      ]),
    ],
