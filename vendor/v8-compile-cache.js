...
class NativeCompileCache {
  ...
  install() {
    const self = this, hasRequireResolvePaths = "function" == typeof require.resolve.paths;
    this._previousModuleCompile = Module.prototype._compile, Module.prototype._compile = function(content, filename) {
      ...
      function resolve(request, options) {
        return Module._resolveFilename(request, mod, !1, options);
      }
      require.resolve = resolve, hasRequireResolvePaths && (resolve.paths = function(request) {
        return Module._resolveLookupPaths(request, mod, !0);
      }), require.main = process.mainModule, require.extensions = Module._extensions, 
      require.cache = Module._cache;
      const dirname = path.dirname(filename), compiledWrapper = self._moduleCompile(filename, content), args = [ mod.exports, require, mod, filename, dirname, process, global, Buffer ];
...
function getMainName() {
  return require.main && "string" == typeof require.main.filename ? require.main.filename : process.cwd();
}

if (!process.env.DISABLE_V8_COMPILE_CACHE && supportsCachedData()) {
  const blobStore = new FileSystemBlobStore(getCacheDir(), getMainName()), nativeCompileCache = new NativeCompileCache;
  nativeCompileCache.setCacheStore(blobStore), nativeCompileCache.install(), process.once("exit", () => {
