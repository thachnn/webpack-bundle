!function() {
  ...
  __webpack_require__.r(__webpack_exports__), __webpack_require__.d(__webpack_exports__, {
    ...
    editWithAST: function() {
      return editWithAST;
    }
  });
  var external_wasm_parser_namespaceObject = require("./wasm-parser"), external_wasm_ast_namespaceObject = require("./wasm-ast"), external_long_namespaceObject = require("./long"), external_long_default = __webpack_require__.n(external_long_namespaceObject);
  ...
  function encodeF32(v) {
    var buffer = [];
    return write(buffer, v, 0, !0, 23, 4), buffer;
  }
  function encodeF64(v) {
    var buffer = [];
    return write(buffer, v, 0, !0, 52, 8), buffer;
  }
  ...
  function _encode(arr) {
    ...
    if (n < 65536) return [ 224 | n >>> 12, con(n >>> 6), con(n) ].concat(_toConsumableArray(_encode(ns)));
    if (n < 1114112) return [ 240 | n >>> 18, con(n >>> 12), con(n >>> 6), con(n) ].concat(_toConsumableArray(_encode(ns)));
  ...
  function encoder_encodeU32(v) {
    var uint8view = new Uint8Array(function(v) {
      return esm_leb.encodeUInt32(v);
    }(v));
    return encoder_toConsumableArray(uint8view);
  }
  function encoder_encodeI32(v) {
    var uint8view = new Uint8Array(function(v) {
      return esm_leb.encodeInt32(v);
    }(v));
    return encoder_toConsumableArray(uint8view);
  }
  function encoder_encodeI64(v) {
    var uint8view = new Uint8Array(function(v) {
      return esm_leb.encodeInt64(v);
    }(v));
    return encoder_toConsumableArray(uint8view);
  }
  function encodeVec(elements) {
    return encoder_toConsumableArray(encoder_encodeU32(elements.length)).concat(encoder_toConsumableArray(elements));
  }
  function encodeValtype(v) {
    var byte = esm.valtypesByString[v];
    if (void 0 === byte) throw new Error("Unknown valtype: " + v);
    return parseInt(byte, 10);
  }
  function encodeMutability(v) {
    var byte = esm.globalTypesByString[v];
  ...
  function encodeLimits(n) {
    var out = [];
    return "number" == typeof n.max ? (out.push(1), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.min))), 
    out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.max)))) : (out.push(0), 
    out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.min)))), out;
  }
  ...
  function encodeNode(n) {
    switch (n.type) {
      ...
      return function(n) {
        ...
        out.push.apply(out, encoder_toConsumableArray(encodeUTF8Vec(n.name))), n.descr.type) {
         ...
         case "FuncImportDescr":
          out.push(0), assertNotIdentifierNode(n.descr.id), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.descr.id.value)));
          break;
     ...
     case "SectionMetadata":
      return function(n) {
        var out = [], sectionId = esm.sections[n.section];
        ...
        return out.push(sectionId), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.size.value))), 
        out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.vectorOfSize.value))), 
        out;
      }(n);
     ...
     case "CallInstruction":
      return function(n) {
        var out = [];
        return assertNotIdentifierNode(n.index), out.push(16), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.index.value))), 
        out;
      }(n);
     ...
     case "CallIndirectInstruction":
      return function(n) {
        var out = [];
        return assertNotIdentifierNode(n.index), out.push(17), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.index.value))), 
        out.push(0), out;
      }(n);
     ...
     case "Instr":
      return function(n) {
        ...
        var byteString = esm.symbolsByName[instructionName];
        ...
        return out.push(byte), n.args && n.args.forEach((function(arg) {
          var encoder = encoder_encodeU32;
          if ("i32" === n.object && (encoder = encoder_encodeI32), "i64" === n.object && (encoder = encoder_encodeI64), 
          "f32" === n.object && (encoder = encodeF32), "f64" === n.object && (encoder = encodeF64), 
     ...
     case "ModuleExport":
      return function(n) {
        ...
        var exportTypeByteString = esm.exportTypesByName[n.descr.exportType];
        ...
        out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.descr.id.value))), 
        out;
      }(n);
     ...
     case "IndexInFuncSection":
      return function(n) {
        return assertNotIdentifierNode(n.index), encoder_encodeU32(n.index.value);
      }(n);
     ...
     case "Elem":
      return function(n) {
        var out = [];
        assertNotIdentifierNode(n.table), out.push.apply(out, encoder_toConsumableArray(encoder_encodeU32(n.table.value))), 
        out.push.apply(out, encoder_toConsumableArray(encodeExpr(n.offset)));
        var funcs = n.funcs.reduce((function(acc, x) {
          return encoder_toConsumableArray(acc).concat(encoder_toConsumableArray(encoder_encodeU32(x.value)));
        }), []);
  ...
  var esm_encodeU32 = encoder_encodeU32;
  ...
  function shrinkPaddedLEB128(ast, uint8Buffer) {
    return (0, external_wasm_ast_namespaceObject.traverse)(ast, {
      SectionMetadata: function(_ref2) {
        var node = _ref2.node, newu32Encoded = esm_encodeU32(node.size.value), newu32EncodedLen = newu32Encoded.length, start = node.size.loc.start.column, end = node.size.loc.end.column, oldu32EncodedLen = end - start;
        if (newu32EncodedLen !== oldu32EncodedLen) {
          ...
          uint8Buffer = overrideBytesInBuffer(uint8Buffer, start, end, newu32Encoded), function(ast, _ref, deltaInSizeEncoding) {
            ...
            (0, external_wasm_ast_namespaceObject.traverse)(ast, {
              SectionMetadata: function(path) {
                path.node.section !== section ? !0 === encounteredSection && (0, external_wasm_ast_namespaceObject.shiftSection)(ast, path.node, deltaInSizeEncoding) : encounteredSection = !0;
              }
            });
  ...
  function createEmptySection(ast, uint8Buffer, section) {
    var start, end, lastSection = function(ast, forSection) {
      for (var lastSection, targetSectionId = esm.sections[forSection], moduleSections = ast.body[0].metadata.sections, lastId = 0, i = 0, len = moduleSections.length; i < len; i++) {
        var section = moduleSections[i];
        if ("custom" !== section.section) {
          var sectionId = esm.sections[section.section];
    ...
    var sizeStartLoc = {
      ...
      line: -1,
      column: start + 1
    }, size = external_wasm_ast_namespaceObject.withLoc(external_wasm_ast_namespaceObject.numberLiteralFromRaw(1), sizeEndLoc, sizeStartLoc), vectorOfSizeStartLoc = {
      line: -1,
      column: sizeEndLoc.column
    }, vectorOfSizeEndLoc = {
      line: -1,
      column: sizeEndLoc.column + 1
    }, vectorOfSize = external_wasm_ast_namespaceObject.withLoc(external_wasm_ast_namespaceObject.numberLiteralFromRaw(0), vectorOfSizeEndLoc, vectorOfSizeStartLoc), sectionMetadata = external_wasm_ast_namespaceObject.sectionMetadata(section, start, size, vectorOfSize), sectionBytes = encodeNode(sectionMetadata);
    ...
    external_wasm_ast_namespaceObject.sortSectionMetadata(ast.body[0]));
    var deltaBytes = +sectionBytes.length, encounteredSection = !1;
    return external_wasm_ast_namespaceObject.traverse(ast, {
      SectionMetadata: function(path) {
        path.node.section !== section ? !0 === encounteredSection && external_wasm_ast_namespaceObject.shiftSection(ast, path.node, deltaBytes) : encounteredSection = !0;
      }
    }), {
  ...
  function shiftLocNodeByDelta(node, delta) {
    (0, external_wasm_ast_namespaceObject.assertHasLoc)(node), node.loc.start.column += delta, 
    node.loc.end.column += delta;
  }
  function applyDelete(ast, uint8Buffer, node) {
    if ((0, external_wasm_ast_namespaceObject.assertHasLoc)(node), "start" === getSectionForNode(node)) {
      var sectionMetadata = (0, external_wasm_ast_namespaceObject.getSectionMetadata)(ast, "start");
      return uint8Buffer = function(ast, uint8Buffer, section) {
        var sectionMetadatas = (0, external_wasm_ast_namespaceObject.getSectionMetadatas)(ast, section);
        ...
        return sectionMetadatas.reverse().reduce((function(uint8Buffer, sectionMetadata) {
          ...
          return (0, external_wasm_ast_namespaceObject.traverse)(ast, {
            SectionMetadata: function(path) {
              ...
              !0 === encounteredSection && (0, external_wasm_ast_namespaceObject.shiftSection)(ast, path.node, delta);
            }
          }), overrideBytesInBuffer(uint8Buffer, startsIncludingId, ends, []);
        }), uint8Buffer);
    ...
    return ops.forEach((function(op) {
      ...
      switch (op.kind) {
       case "update":
        state = function(ast, uint8Buffer, _ref) {
          ...
          (0, external_wasm_ast_namespaceObject.assertHasLoc)(oldNode);
          var sectionName = getSectionForNode(newNode), replacementByteArray = encodeNode(newNode);
          uint8Buffer = overrideBytesInBuffer(uint8Buffer, oldNode.loc.start.column, oldNode.loc.end.column, replacementByteArray), 
          "code" === sectionName && (0, external_wasm_ast_namespaceObject.traverse)(ast, {
            Func: function(_ref3) {
              var node = _ref3.node;
              if (!0 == (void 0 !== node.body.find((function(n) {
                return n === newNode;
              })))) {
                (0, external_wasm_ast_namespaceObject.assertHasLoc)(node);
                ...
                if (0 !== bodySizeDeltaBytes) {
                  var newValue = node.metadata.bodySize + bodySizeDeltaBytes, newByteArray = esm_encodeU32(newValue), start = node.loc.start.column;
        ...
        }(ast, uint8Buffer, [ op.oldNode, op.node ]), sectionName = getSectionForNode(op.node);
        break;

       case "delete":
        state = applyDelete(ast, uint8Buffer, op.node), sectionName = getSectionForNode(op.node);
        break;

       case "add":
        state = function(ast, uint8Buffer, node) {
          var body, sectionName = getSectionForNode(node), sectionMetadata = (0, external_wasm_ast_namespaceObject.getSectionMetadata)(ast, sectionName);
          ...
          if ((0, external_wasm_ast_namespaceObject.isFunc)(node) && (0 === (body = node.body).length || "end" !== body[body.length - 1].id)) throw new Error("expressions must be ended");
          if ((0, external_wasm_ast_namespaceObject.isGlobal)(node) && (0 === (body = node.init).length || "end" !== body[body.length - 1].id)) throw new Error("expressions must be ended");
          var newByteArray = encodeNode(node), start = (0, external_wasm_ast_namespaceObject.getEndOfSection)(sectionMetadata), end = start, deltaBytes = newByteArray.length;
          ...
          return "IndexInFuncSection" !== node.type && (0, external_wasm_ast_namespaceObject.orderedInsertNode)(ast.body[0], node), 
          {
            uint8Buffer: uint8Buffer,
            deltaBytes: deltaBytes,
            deltaElements: 1
          };
        }(ast, uint8Buffer, op.node), sectionName = getSectionForNode(op.node);
        break;
      ...
      if (0 !== state.deltaElements && "start" !== sectionName) {
        ...
        state.uint8Buffer = function(ast, uint8Buffer, section, deltaElements) {
          var sectionMetadata = (0, external_wasm_ast_namespaceObject.getSectionMetadata)(ast, section);
      ...
      if (0 !== state.deltaBytes && "start" !== sectionName) {
        ...
        state.uint8Buffer = function(ast, uint8Buffer, section, deltaBytes) {
          ...
          return (0, external_wasm_ast_namespaceObject.traverse)(ast, {
            SectionMetadata: function(path) {
              path.node.section !== section ? !0 === encounteredSection && (0, external_wasm_ast_namespaceObject.shiftSection)(ast, path.node, deltaBytes) : encounteredSection = !0;
            }
          }), overrideBytesInBuffer(uint8Buffer, start, end, newBytes);
  ...
  function preprocess(ab) {
    return function(uint8Buffer) {
      try {
        return shrinkPaddedLEB128((0, external_wasm_parser_namespaceObject.decode)(uint8Buffer.buffer, decoderOpts), uint8Buffer);
      } catch (e) {
  ...
  function edit(ab, visitors) {
    return ab = preprocess(ab), editWithAST((0, external_wasm_parser_namespaceObject.decode)(ab), ab, visitors);
  }
  function editWithAST(ast, ab, visitors) {
    var nodeBefore, operations = [], uint8Buffer = new Uint8Array(ab);
    return (0, external_wasm_ast_namespaceObject.traverse)(ast, visitors, (function(type, path) {
      nodeBefore = (0, external_wasm_ast_namespaceObject.cloneNode)(path.node);
    }), (function(type, path) {
  ...
  function add(ab, newNodes) {
    return ab = preprocess(ab), addWithAST((0, external_wasm_parser_namespaceObject.decode)(ab), ab, newNodes);
  }
  function addWithAST(ast, ab, newNodes) {
    !function(nodes) {
      ...
      nodes.sort((function(a, b) {
        var sectionA = getSectionForNode(a), sectionB = getSectionForNode(b), aId = esm.sections[sectionA], bId = esm.sections[sectionB];
