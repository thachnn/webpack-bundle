...
module.exports = function(modules) {
  ...
  module.exports = function(yargs, usage, validation, globalMiddleware) {
    ...
    return globalMiddleware = globalMiddleware || [], self.addHandler = function(cmd, description, builder, handler, commandMiddleware) {
      let aliases = [];
      ...
      }), !1 !== description && usage.command(cmd, description, isDefault, aliases), handlers[parsedCommand.cmd] = {
        original: cmd,
    ...
    self.runCommand = function(command, yargs, parsed, commandIndex) {
      ...
      !1 === yargs.parsed ? (shouldUpdateUsage(yargs) && yargs.getUsageInstance().usage(usageFromParentCommandsCommandHandler(parentCommands, commandHandler), commandHandler.description), 
      innerArgv = innerYargs ? innerYargs._parseArgs(null, null, !0, commandIndex) : yargs._parseArgs(null, null, !0, commandIndex)) : innerArgv = yargs.parsed.argv, 
      aliases = innerYargs && !1 === yargs.parsed ? innerYargs.parsed.aliases : yargs.parsed.aliases) : "object" == typeof commandHandler.builder && (innerYargs = yargs.reset(parsed.aliases), 
      ...
      commandHandler.handler && !yargs._hasOutput()) {
        yargs._setHasOutput(), innerArgv = applyMiddleware(innerArgv, yargs, middlewares, !1);
    ...
    self.freeze = () => {
      frozen = {}, frozen.handlers = handlers, frozen.aliasMap = aliasMap, frozen.defaultCommand = defaultCommand;
    }, self.unfreeze = () => {
  ...
  module.exports = {
    applyMiddleware: function(argv, yargs, middlewares, beforeValidation) {
      ...
      return middlewares.reduce((accumulation, middleware) => {
        if (middleware.applyBeforeValidation !== beforeValidation && !isPromise(accumulation)) return accumulation;
  ...
  function parse(args, opts) {
    ...
    Object.keys(flags.bools).forEach((function(key) {
      Object.prototype.hasOwnProperty.call(defaults, key) && (setArg(key, defaults[key]), 
      function(key) {
    ...
    for (var notFlags = [], i = 0; i < args.length; i++) {
      var broken, key, letters, m, next, value, arg = args[i];
      if (arg.match(/^--.+=/) || !configuration["short-option-groups"] && arg.match(/^-.+=/)) checkAllAliases((m = arg.match(/^--?([^=]+)=([\s\S]*)$/))[1], flags.nargs) ? (args.splice(i + 1, 0, m[2]), 
      i = eatNargs(i, m[1], args)) : checkAllAliases(m[1], flags.arrays) && args.length > i + 1 ? (args.splice(i + 1, 0, m[2]), 
      i = eatArray(i, m[1], args)) : setArg(m[1], m[2]); else if (arg.match(negatedBoolean) && configuration["boolean-negation"]) setArg(key = arg.match(negatedBoolean)[1], !1); else if (arg.match(/^--.+/) || !configuration["short-option-groups"] && arg.match(/^-[^-]+/)) checkAllAliases(key = arg.match(/^--?(.+)/)[1], flags.nargs) ? i = eatNargs(i, key, args) : checkAllAliases(key, flags.arrays) && args.length > i + 1 ? i = eatArray(i, key, args) : void 0 === (next = 0 === flags.nargs[key] ? void 0 : args[i + 1]) || next.match(/^-/) && !next.match(negative) || checkAllAliases(key, flags.bools) || checkAllAliases(key, flags.counts) ? /^(true|false)$/.test(next) ? (setArg(key, next), 
      ...
      i++); else if (arg.match(/^-[^-]+/) && !arg.match(negative)) {
        ...
        for (var j = 0; j < letters.length; j++) {
          if (next = arg.slice(j + 2), letters[j + 1] && "=" === letters[j + 1]) {
            ...
            i = eatNargs(i, key, args)) : checkAllAliases(key, flags.arrays) && args.length > i + 1 ? (args.splice(i + 1, 0, value), 
        ...
        key = arg.slice(-1)[0], broken || "-" === key || (checkAllAliases(key, flags.nargs) ? i = eatNargs(i, key, args) : checkAllAliases(key, flags.arrays) && args.length > i + 1 ? i = eatArray(i, key, args) : void 0 === (next = args[i + 1]) || /^(-|--)[^-]/.test(next) && !next.match(negative) || checkAllAliases(key, flags.bools) || checkAllAliases(key, flags.counts) ? /^(true|false)$/.test(next) ? (setArg(key, next), 
    ...
    function eatArray(i, key, args) {
      for (var start = i + 1, argsToSet = [], multipleArrayFlag = i > 0, ii = i + 1; ii < args.length; ii++) {
        if (/^-/.test(args[ii]) && !negative.test(args[ii])) {
          ii === start && setArg(key, defaultForType("array")), multipleArrayFlag = !0;
          break;
        }
        i = ii, argsToSet.push(args[ii]);
      }
      return multipleArrayFlag ? setArg(key, argsToSet.map((function(arg) {
    ...
    function setArg(key, val) {
      ...
      }(key), /-/.test(key) && configuration["camel-case-expansion"]) {
    ...
    function setConfigObject(config, prev) {
      Object.keys(config).forEach((function(key) {
        ...
        "object" == typeof value && null !== value && !Array.isArray(value) && configuration["dot-notation"] ? setConfigObject(value, fullKey) : (!hasKey(argv, fullKey.split(".")) || flags.defaulted[fullKey] || flags.arrays[fullKey] && configuration["combine-arrays"]) && setArg(fullKey, value);
      }));
    }
      ...
      if (void 0 !== envPrefix) {
        ...
        Object.keys(process.env).forEach((function(envVar) {
          if ("" === prefix || 0 === envVar.lastIndexOf(prefix, 0)) {
            ...
            !(configOnly && flags.configs[keys.join(".")] || !configOnly) || hasKey(argv, keys) && !flags.defaulted[keys.join(".")] || setArg(keys.join("."), process.env[envVar]);
          }
        }));
      }
    }
    ...
    function setKey(obj, keys, value) {
      ...
      value === increment ? o[key] = increment(o[key]) : Array.isArray(o[key]) ? duplicate && isTypeArray && isValueArray ? o[key] = configuration["flatten-duplicate-arrays"] ? o[key].concat(value) : (Array.isArray(o[key][0]) ? o[key] : [ o[key] ]).concat([ value ]) : duplicate || Boolean(isTypeArray) !== Boolean(isValueArray) ? o[key] = o[key].concat([ value ]) : o[key] = value : void 0 === o[key] && isTypeArray ? o[key] = isValueArray ? value : [ value ] : duplicate && !(void 0 === o[key] || checkAllAliases(key, flags.bools) || checkAllAliases(keys.join("."), flags.bools) || checkAllAliases(key, flags.counts)) ? o[key] = [ o[key], value ] : o[key] = value;
    }
    ...
    function defaultValue(key) {
      return checkAllAliases(key, flags.bools) || checkAllAliases(key, flags.counts) || !("" + key in defaults) ? defaultForType(function(key) {
        var type = "boolean";
        checkAllAliases(key, flags.strings) ? type = "string" : checkAllAliases(key, flags.numbers) ? type = "number" : checkAllAliases(key, flags.arrays) && (type = "array");
        return type;
      }(key)) : defaults[key];
    }
  ...
  (module.exports = function(processArgs, cwd, parentRequire) {
    ...
    let options, frozen;
    function freeze() {
      frozen = {}, frozen.options = options, frozen.configObjects = options.configObjects.slice(0), 
    ...
    function unfreeze() {
      ...
      strict = frozen.strict, completionCommand = frozen.completionCommand, parseFn = null, 
      parseContext = null, frozen = void 0;
    }
    ...
    self.getContext = () => context, self.resetOptions = self.reset = function(aliases) {
    ...
    }, self.resetOptions(), self.boolean = function(keys) {
      ...
      "object" == typeof key ? (key = applyExtends(key, cwd), options.configObjects = (options.configObjects || []).concat(key), 
      self) : ("function" == typeof msg && (parseFn = msg, msg = null), key = key || "config", 
      self.describe(key, msg || usage.deferY18nLookup("Path to JSON config file")), (Array.isArray(key) ? key : [ key ]).forEach(k => {
    ...
    options.demandedCommands), self.implies = function(key, value) {
      ...
      return obj[key] && "object" == typeof obj[key] && (conf = applyExtends(obj[key], rootPath || cwd), 
    ...
    self.parse = function(args, shortCircuit, _parseFn) {
      ...
      void 0 === args) return self._parseArgs(processArgs);
      ...
      shortCircuit || (processArgs = args), freeze(), parseFn && (exitProcess = !1);
    ...
    self.completion = function(cmd, desc, fn) {
      ...
      "function" == typeof desc && (fn = desc, desc = null), completionCommand = cmd || "completion", 
      ...
      return argsert("[string]", [ $0 ], arguments.length), $0 = $0 || self.$0, _logger.log(completion.generateCompletionScript($0, completionCommand)), 
      self;
    ...
    function guessLocale() {
      if (detectLocale) try {
        const {env: env} = process, locale = env.LC_ALL || env.LC_MESSAGES || env.LANG || env.LANGUAGE || "en_US";
    ...
    void 0 !== process.stdout.columns ? process.stdout.columns : null), Object.defineProperty(self, "argv", {
      ...
      pkgConfig && (console.warn("Configuring yargs through package.json is deprecated and will be removed in the next major release, please use the JS API instead."), 
      ...
      try {
        if (guessLocale(), shortCircuit) return argv;
        if (helpOpt) {
        ...
        if (argv._.length) {
          if (handlerKeys.length) {
            ...
            for (let cmd, i = commandIndex || 0; void 0 !== argv._[i]; i++) {
              if (cmd = String(argv._[i]), ~handlerKeys.indexOf(cmd) && cmd !== completionCommand) return command.runCommand(cmd, self, parsed, i + 1);
              if (!firstUnknownCommand && cmd !== completionCommand) {
                firstUnknownCommand = cmd;
                break;
              }
            }
            if (command.hasDefaultCommand() && !skipDefaultCommand) return command.runCommand(null, self, parsed);
        ...
        } else if (command.hasDefaultCommand() && !skipDefaultCommand) return command.runCommand(null, self, parsed);
  ...
  const camelCase = (input, options) => {
    ...
    if (0 === (input = Array.isArray(input) ? input.map(x => x.trim()).filter(x => x.length).join("-") : input.trim()).length) return "";
  ...
  module.exports = function(yargs, usage, command) {
    ...
    }, zshShell = process.env.SHELL && -1 !== process.env.SHELL.indexOf("zsh");
    self.getCompletion = function(args, done) {
  ...
  module.exports = function(yargs, y18n) {
    ...
    let epilog, descriptions = {};
    self.describe = function(key, desc) {
      "object" == typeof key ? Object.keys(key).forEach(k => {
        self.describe(k, key[k]);
      }) : descriptions[key] = desc;
    }, self.getDescriptions = () => descriptions, self.epilog = msg => {
      epilog = msg;
    };
    let wrap, wrapSet = !1;
    function getWrap() {
      return wrapSet || (wrap = "object" == typeof process && process.stdout && process.stdout.columns ? Math.min(80, process.stdout.columns) : 80, 
      wrapSet = !0), wrap;
    }
    self.wrap = cols => {
      wrapSet = !0, wrap = cols;
    ...
    self.help = function() {
      ...
      const base$0 = path.basename(yargs.$0), demandedOptions = yargs.getDemandedOptions(), demandedCommands = yargs.getDemandedCommands(), groups = yargs.getGroups(), options = yargs.getOptions();
      let keys = [];
    ...
    return self.version = ver => {
      ...
      epilog = void 0, examples = [], commands = [], descriptions = objFilter(descriptions, (k, v) => !localLookup[k]), 
      self;
    }, self.freeze = function() {
      frozen = {}, frozen.failMessage = failMessage, frozen.failureOutput = failureOutput, 
      frozen.usages = usages, frozen.usageDisabled = usageDisabled, frozen.epilog = epilog, 
      ...
      usageDisabled = frozen.usageDisabled, epilog = frozen.epilog, examples = frozen.examples, 
      commands = frozen.commands, descriptions = frozen.descriptions, frozen = void 0;
    }, self;
  };
}, function(module, exports, __webpack_require__) {
  ...
  module.exports = function(yargs, usage, y18n) {
    const __ = y18n.__, __n = y18n.__n, self = {
      ...
      unknownArguments: function(argv, aliases, positionalMap) {
        ...
        Object.keys(argv).forEach(key => {
          -1 !== specialKeys.indexOf(key) || positionalMap.hasOwnProperty(key) || yargs._getParseContext().hasOwnProperty(key) || aliases.hasOwnProperty(key) || unknown.push(key);
        }), commandKeys.length > 0 && argv._.slice(currentContext.commands.length).forEach(key => {
    ...
    self.implies = function(key, value) {
      ...
      if (Object.keys(implied).forEach(key => {
        ...
        (implied[key] || []).forEach(value => {
          ...
          num = Number(key), key = isNaN(num) ? key : num, "number" == typeof key ? key = argv._.length >= key : key.match(/^--no-.+/) ? (key = key.match(/^--no-(.+)/)[1], 
          key = !argv[key]) : key = argv[key], num = Number(value), "number" == typeof (value = isNaN(num) ? value : num) ? value = argv._.length >= value : value.match(/^--no-.+/) ? (value = value.match(/^--no-(.+)/)[1], 
          value = !argv[value]) : value = argv[value], key && !value && implyFail.push(` ${origKey} -> ${origValue}`);
        });
      }), implyFail.length) {
    ...
    let frozen, conflicting = {};
    return self.conflicts = function(key, value) {
