...
/* (c) James Talmage <james@talmage.io> (github.com/jamestalmage) */
var _nodeModulesRegexp = /^(?:.*[\\\/])?node_modules(?:[\\\/].*)?$/;

// Guard against poorly mocked module constructors.
var Module = module.constructor.length > 1 ? module.constructor : _module;
...
function shouldCompile(filename, exts, matcher, ignoreNodeModules) {
  ...
  if (exts.indexOf(_path.extname(filename)) === -1) return false;

  var resolvedFilename = _path.resolve(filename);

  if (ignoreNodeModules && _nodeModulesRegexp.test(resolvedFilename)) return false;
