...
var BIND_NONE = 0, BIND_VAR = 1, BIND_LEXICAL = 2, BIND_FUNCTION = 3, BIND_SIMPLE_CATCH = 4, BIND_OUTSIDE = 5, Parser = function(options, input, startPos) {
  ...
  this.scopeStack = [], this.enterScope(SCOPE_TOP), this.regexpState = null;
}, prototypeAccessors = {
  inFunction: {
    configurable: !0
  },
...
pp$1.parseTryStatement = function(node) {
  if (this.next(), node.block = this.parseBlock(), node.handler = null, this.type === types._catch) {
    ...
    if (this.next(), this.eat(types.parenL)) {
      ...
      this.enterScope(simple ? SCOPE_SIMPLE_CATCH : 0), this.checkLVal(clause.param, simple ? BIND_SIMPLE_CATCH : BIND_LEXICAL), 
  ...
  decl.id = this.parseBindingAtom(), this.checkLVal(decl.id, "var" === kind ? BIND_VAR : BIND_LEXICAL, !1);
};
...
pp$1.parseFunction = function(node, statement, allowExpressionBody, isAsync) {
  ...
  statement & FUNC_STATEMENT && (node.id = statement & FUNC_NULLABLE_ID && this.type !== types.name ? null : this.parseIdent(), 
  !node.id || statement & FUNC_HANGING_STATEMENT || this.checkLVal(node.id, this.strict || node.generator || node.async ? this.treatFunctionsAsVar ? BIND_VAR : BIND_LEXICAL : BIND_FUNCTION));
  ...
  this.type === types.name ? (node.id = this.parseIdent(), isStatement && this.checkLVal(node.id, BIND_LEXICAL, !1)) : (!0 === isStatement && this.unexpected(), 
  node.id = null);
  ...
  if (this.type === types.name) {
    var node = this.startNode();
    if (node.local = this.parseIdent(), this.checkLVal(node.local, BIND_LEXICAL), nodes.push(this.finishNode(node, "ImportDefaultSpecifier")), 
    !this.eat(types.comma)) return nodes;
  }
  if (this.type === types.star) {
    var node$1 = this.startNode();
    return this.next(), this.expectContextual("as"), node$1.local = this.parseIdent(), 
    this.checkLVal(node$1.local, BIND_LEXICAL), nodes.push(this.finishNode(node$1, "ImportNamespaceSpecifier")), 
    nodes;
  }
  for (this.expect(types.braceL); !this.eat(types.braceR); ) {
    if (first) first = !1; else if (this.expect(types.comma), this.afterTrailingComma(types.braceR)) break;
    ...
    node$2.local = node$2.imported), this.checkLVal(node$2.local, BIND_LEXICAL), nodes.push(this.finishNode(node$2, "ImportSpecifier"));
  }
  return nodes;
...
pp$2.toAssignable = function(node, isBinding, refDestructuringErrors) {
  ...
  switch (void 0 === bindingType && (bindingType = BIND_NONE), expr.type) {
   case "Identifier":
    bindingType === BIND_LEXICAL && "let" === expr.name && this.raiseRecoverable(expr.start, "let is disallowed as a lexically bound name"), 
    ...
    checkClashes[expr.name] = !0), bindingType !== BIND_NONE && bindingType !== BIND_OUTSIDE && this.declareName(expr.name, bindingType, expr.start);
    break;
...
pp$3.parseNew = function() {
  ...
  this.exitScope(), this.strict && node.id && this.checkLVal(node.id, BIND_OUTSIDE), 
...
RegExpValidationState.prototype.reset = function(start, pattern, flags) {
  ...
  if (state.eat(117)) {
    ...
    if (state.switchU && state.eat(123) && this.regexp_eatHexDigits(state) && state.eat(125) && isValidUnicode(state.lastIntValue)) return !0;
...
exports.isIdentifierStart = isIdentifierStart, exports.isNewLine = isNewLine, exports.keywordTypes = keywords$1, 
exports.lineBreak = lineBreak, exports.lineBreakG = lineBreakG, exports.nonASCIIwhitespace = nonASCIIwhitespace, 
exports.parse = parse, exports.parseExpressionAt = parseExpressionAt, exports.tokContexts = types$1, 
exports.tokTypes = types, exports.tokenizer = tokenizer, exports.version = version, 
