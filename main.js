(() => {
  ...
  const source_map_namespaceObject = require("./lib/source-map");
  ...
  async function minify(files, options, _fs_module) {
    ...
    !HOP(options.format, "code") || options.format.code) {
      if (options.sourceMap && (options.format.source_map = await async function(options) {
        ...
        var generator = new source_map_namespaceObject.SourceMapGenerator({
          file: options.file,
          sourceRoot: options.root
        });
        return options.orig && (orig_map = await new source_map_namespaceObject.SourceMapConsumer(options.orig)).sources.forEach((function(source) {
    ...
    filesList ? function simple_glob(glob) {
      ...
      try {
        program.parse && (program.parse.acorn ? files = convert_ast((function(toplevel, name) {
          return require("./lib/acorn").parse(files[name], {
            ecmaVersion: 2018,
            locations: !0,
            program: toplevel,
            sourceFile: name,
