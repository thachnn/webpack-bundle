(() => {
  ...
  var __webpack_modules__ = {
    ...
    553: (module, __unused_webpack_exports, __webpack_require__) => {
      const path = __webpack_require__(17), findUp = __webpack_require__(270), pkgDir = async cwd => {
    ...
    516: (module, __unused_webpack_exports, __webpack_require__) => {
      ...
      module.exports = filename => {
        const normalizedFilename = filename.startsWith("file://") ? fileURLToPath(filename) : filename, globalDir = pkgDir.sync(path.dirname(normalizedFilename)), relativePath = path.relative(globalDir, normalizedFilename), pkg = __webpack_require__(965)(path.join(globalDir, "package.json")), localFile = resolveCwd.silent(path.join(pkg.name, relativePath)), localNodeModules = path.join(process.cwd(), "node_modules");
        return !(!path.relative(localNodeModules, normalizedFilename).startsWith("..") && path.parse(localNodeModules).root === path.parse(normalizedFilename).root) && localFile && "" !== path.relative(localFile, normalizedFilename) && __webpack_require__(965)(localFile);
      };
    },
    965: module => {
